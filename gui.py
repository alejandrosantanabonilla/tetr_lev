...
class LevGUI(ModelessDialog):
    ...
    #code common to the Tetr and Lev00
...
#################### Create console output widget and label (LHS of window) #####################
        ...
        #make resizable in the x direction
        ...
        t=Tk.Text(consoleFrame, wrap=Tk.WORD,width=80,
                  height=45,yscrollcommand=scrollbar.set)
        t.config(font=('courier', 12, 'normal'))
        t.grid(row=row,column=0, sticky=Tk.W+Tk.E+Tk.N+Tk.S)
        consoleFrame.rowconfigure(row,weight=1)

        #make resizable in the y direction
        scrollbar.grid(row=row, column=1, sticky='ns')
        scrollbar.config(command=t.yview)
        
        self.outText = t
        
        #allow a button press to focus the text box
        #(allows selection of text from concole output) 
...
#################### text entry widget (bottom of screen) #####################
        ...
        Tk.Label(parent, text=self.App+" Input:").grid(row=row,
                                                       sticky=Tk.W,padx=5)
        row += 1
        ...
        Tk.Button(parent,text="<- Enter Command",
                  command=self._updateInput).grid(row=row,
                                                  column=1,
...
#--------frame for view options (ball and stick, wireframe etc)---------
        ...
        Tk.Label(viewFrame, text="Display Options:").grid(row=framerow,
                                                          sticky=Tk.W)
        ...
        framerow+=1
        Tk.Checkbutton(viewFrame,text="Show Numbers"
                       ,variable=self.showNumber,command=self.UpdateLabels).grid(row=framerow,
                                                                                 column=framecol,sticky=Tk.W)
        
        framerow+=1
        Tk.Checkbutton(viewFrame,text="Show Species"
                       ,variable=self.showAtom,command=self.UpdateLabels).grid(row=framerow,
                                                                               column=framecol,sticky=Tk.W)
...
#--------frame for axis options---------        
        ...
        Tk.Checkbutton(axisFrame,text="Toggle Axis",variable=self.axisshow,
                       command=self.axis_toggle).grid(row=axisRow,columnspan=2,sticky=Tk.W)
        ...
        self.LatVecButton=Tk.Checkbutton(axisFrame,text="Toggle Vectors (Vs)",
                                         variable=self.lattice,
                                         command=self.ToggleLatticeVectors)
...
#-------frame for console display options ------------
        ...
        for txt,val in dispoptions:
            framerow+=1
            Tk.Radiobutton(toptFrame,
                           text=txt,padx=20,
                           variable=self.DispOption,
                           command=self.SwitchConsoleView,value=val).grid(row=framerow,
                                                                          column=framecol,sticky=Tk.W)
...
#--------frame for atom selections---------  
        ...
        if self.App=="Tetr":
            ...
            Tk.Button(selectionFrame,text="Get Selection",
                      command=self.GetSelection).grid(row=1,column=0, sticky=Tk.W+Tk.E)
            Tk.Button(selectionFrame,text="?",
                      command=self.selectionHelp).grid(row=1,column=1, sticky=Tk.W+Tk.E)
...
#--------frame for reset controls---------
        ...
        Tk.Label(resetFrame,text="Restart "+self.App+":").grid(row=0,
                                                               column=0,columnspan=2,sticky=Tk.W)
        Tk.Button(resetFrame,text="Restart",command=self.reset_app).grid(row=1,
                                                                         column=0,sticky=Tk.W+Tk.E)
        Tk.Button(resetFrame,text="?",command=self.resetHelp).grid(row=1,
        ...
        if self.lattice.get() == 1:
            ...
            if not os.path.isfile(filename):
                tkMessageBox.showwarning("Warning",
                                         "No vectors file (vectors.vct) present. Create this using the 'Vs' option in "+self.App)
                self.lattice.set(0)
                return
    ...
    def GetSelection(self):
        ...
        #gets the atom selections from chimera and pastes the text into
        #the input box 

        length=self.getmaxlen()
        if length <=0:
            length=10000
            print("Warning cannot find length - resorting to default of 10000")
    ...
    def getmaxlen(self):
        ...
        try:
            ...
            try:
                ...
                print("Error, no real8.inc or real8.f90 files found!")
        ...
        for line in f:
            ...
            if pos >=0: #if we found this in a line...
                ...
                for i in range(len(ln)):
                    #read in number till the end of the number
                    if not ln[i].isdigit():
                        break
                num=ln[0:i]
                length=int(num)
    ...
    def make_axis(self):
        ...
        f=open(axisfile,"w")
        #f.write(".sphere "+str(xs)+" "+str(ys)+" "+str(zs)+"
    ...
    def chooseShell(self):
        ...
        self.label=Tk.Label(win,
                            text="Please choose the shell you would like to execute "+self.App+" in:")
        ...
        Tk.Button(win,text="Confirm",
                  command=lambda: self.setShell(option.get(),win)).grid(row=row)
    ...
    #try to find the Tetr/Lev00 root directory.
    #If not, ask the user to give it
        if self.App=="Tetr":
        ...
        # if the tetr/lev00 root file exists, use the path in it,
    ...
    def getWorkingDir(self):
        #ask the user to set the working directory. Choose from
        #the previously used one, or choose a new one
...
class Lev00Dialog(LevGUI):
    ...
    def __init__(self,sessionData=None, *args, **kw):
        ...
        self.pausetime=0.5
        #time to wait between submitting a
        #command and checking for output/refreshing the text (seconds)
    ...
    def CheckforCube(self):
        #enables the cube file vewing option
        #when a new cube file has been created
